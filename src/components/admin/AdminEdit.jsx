...
const EditBody = ({ page }) => {
  ...
  return (
    <div style={{ position: 'relative', display: 'flex' }}>
      <button style={{ position: 'relative', top: '50%', padding: '11px', fontSize: '1.2rem' }}
        onMouseDown={(e) => {
          kd = true;
            ...
            if (kd) {
              ...
              interval.push(setInterval(() => {
                if (parseInt(e.target.nextElementSibling.children[0].style.marginLeft) < 15)
                  e.target.nextElementSibling.children[0].style.marginLeft = parseInt(e.target.nextElementSibling.children[0].style.marginLeft) + 4 + 'px';
              }, 10))
            }
          }, 500)
        }}
      ...
      <div style={{ display: 'flex', width: '700px', position: 'relative', overflow: 'hidden' }}>
        {pitem && pitem.map(p => <div key={p} style={{ marginLeft: '15px' }}>{data.map(q => q.id === p ? <img key={q.id} style={{ width: '80px', height: '80px' }} src={q.imgsrc1} alt=''></img> : '')}</div>)}
      </div>
      <button style={{ position: 'relative', right: '0px', top: '50%', padding: '11px', fontSize: '1.2rem' }}
        onMouseDown={(e) => {
          kd = true;
            ...
            if (kd) {
              interval.push(setInterval(() => {
                if (parseInt(e.target.previousElementSibling.children[0].style.marginLeft) > 740 - e.target.previousElementSibling.children.length * 100)
                  e.target.previousElementSibling.children[0].style.marginLeft = parseInt(e.target.previousElementSibling.children[0].style.marginLeft) - 4 + 'px';
              }, 10))
            }
          }, 500)
        }}
