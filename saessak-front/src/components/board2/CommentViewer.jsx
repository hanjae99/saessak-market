    ...
    viewer.nickname !== "" && (
      <div className={parentCommentId === "" ? "cmtInput" : "cmtInput2"}>
        {parentCommentId === "" ? (
          ...
          <CmtBtnBox cs={{ right: "20px" }}>{btns}</CmtBtnBox>
        )}
        <div className={parentCommentId === "" ? "cmt_info" : "cmt_info2"}>
          {parentCommentId === "" ? (
            <div></div>
          ) : (
            <span style={{ color: "#aaa", fontSize: "1.3rem" }}>
              <BsArrowReturnRight />
            </span>
          )}
          <strong>댓글쓰기</strong>
        ...
        <div
          className={
            parentCommentId === "" ? "cmtInput_textbox" : "cmtInput_textbox2"
          }
        >
          ...
          <button
            onClick={(e) => {
              ...
              tmp =
                parentCommentId === "" ? tmp : { ...tmp, pid: parentCommentId };
...
const Comments = ({
  ...
  return (
    <>
      {commentData.list
        .filter((p) => p.pid + "" === parentCommentId + "")
        .map((p) => (
          <div
            key={p.id}
          ...
          >
            ...
            >
              {parentCommentId === 0 ? (
                ...
                <span
                  style={{ color: "#aaa", fontSize: "2rem", margin: "0 15px" }}
                >
                  <BsArrowReturnRight />
                </span>
              )}
              ...
              <div>
                ...
                <div className="cmt_hover_button"></div>
                <div className="re_cmt_inputbox" style={{ display: "none" }}>
                  <CmtInputBox
                    viewer={{
                      profileImg: p.writerProfileImgUrl,
...
const CommentViewer = ({ parent, parentId, isAnonymous = false }) => {
  ...
  const viewer = {
    ...
    nickname: commentData.userNickName,
  };

  return (
    <div className="commentBox">
      {commentData.viewerRole !== "any" && (
        <CmtInputBox
          viewer={viewer}
          isAnonymous={isAnonymous}
          parent={parent}
